...
fn add_property() {
    ...
    execute(deps.as_mut(), mock_env(), info, msg).unwrap();
    let d = query_property_info(deps.as_ref(), 1).unwrap();
    assert_eq!(
        d,
        FlatInfo {
            renter: renter.to_string(),
...
fn request_lease() {
    ...
    let err = execute(deps.as_mut(), mock_env(), info, msg).unwrap_err();
    assert_eq!(err, ContractError::NotFound {});
...
fn pay_rent() {
    ...
    let rentee = String::from("rentee");
    let info = mock_info(rentee.as_str(), &coins(200u128, "cudo"));
    let env = mock_env();
    ...
    let msg = ExecuteMsg::PayRent { property_id: 0 };
    let rentee = String::from("rentee");
    let info = mock_info(rentee.as_str(), &coins(100u128, "cudo"));
    let env = mock_env();
    let err = execute(deps.as_mut(), env.clone(), info, msg).unwrap_err();
    assert!(matches!(err, ContractError::Std(StdError::Overflow { .. })));
}
...
fn reject_lease() {
    ...
    let msg = ExecuteMsg::RejectLease { property_id: 0 };
    let renter = String::from("renter");
    let info = mock_info(renter.as_str(), &[]);
    let env = mock_env();
    let err = execute(deps.as_mut(), env.clone(), info, msg).unwrap_err();
    ...
    let msg = ExecuteMsg::RequestForLease { property_id: 0 };
    let rentee = String::from("rentee");
    let info = mock_info(rentee.as_str(), &coins(400u128, "cudo"));
    let env = mock_env();
    execute(deps.as_mut(), env.clone(), info, msg).unwrap();
